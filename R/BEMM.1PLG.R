BEMM.1PLG=function(data, 					#A matrix of respoonse [n.examinees * n.items]
  ...
  Model='1PLG'                #Set the model to be 3PLM
  ...
  res=.C("BEMM1PLG", data.simple=data.simple, CountNum=CountNum, n.class=n.class, J=J,  LH=LH, 
         IBeta=IBeta, IGamma=IGamma, TBeta=TBeta, TGamma=TGamma, 
         deltahat.Beta=deltahat.Beta, deltahat.Gamma=deltahat.Gamma,
         est.Beta=Par.est0$Beta, est.Gamma=Par.est0$Gamma, Tol=Tol, cr=cr, StopNormal=StopNormal,
         se.Beta=Par.SE0$SEBeta, se.Gamma=Par.SE0$SEGamma, ParConstraint=ParConstraint,  
         f=f,  r=r, fz=fz,  rz=rz,  P=P, Pstar=Pstar, AG=AG, LL=LL, LL0=LL0,  Posterior.prob=Posterior.prob,
         PriorBeta=PriorBeta, PriorGamma=PriorGamma, n.Quadpts=n.Quadpts, node.Quadpts=node.Quadpts, 
  ...
  EM.Map=list(Map.Beta=EM.Map.Beta, Map.Gamma=EM.Map.Gamma)  #Transfer the variable EM.map to a dataframe
