BEMM.3PL=function(data, 				#A matrix of respoonse [n.examinees * n.items]
  ...
  res=.C("BEMM3PL", data.simple=data.simple, CountNum=CountNum, n.class=n.class, J=J,  LH=LH, 
         IA=IA, IB=IB, IC=IC, IAB=IAB, TA=TA, TB=TB, TC=TC,
         deltahat.A=deltahat.A, deltahat.B=deltahat.B, deltahat.C=deltahat.C,
         est.A=Par.est0$A, est.B=Par.est0$B, est.C=Par.est0$C, Tol=Tol, cr=cr, StopNormal=StopNormal,
         se.A=Par.SE0$SEA, se.B=Par.SE0$SEB, se.C=Par.SE0$SEC, ParConstraint=ParConstraint,   
         f=f,  r=r, fz=fz,  rz=rz,  P=P, Pstar=Pstar,  LL=LL, LL0=LL0,  Posterior.prob=Posterior.prob,
         PriorA=PriorA, PriorB=PriorB, PriorC=PriorC, n.Quadpts=n.Quadpts, node.Quadpts=node.Quadpts, 
  ...
  EM.Map=list(Map.A=EM.Map.A,Map.B=EM.Map.B, Map.C=EM.Map.C) #Transfer the variable EM.map to a list
