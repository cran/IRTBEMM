...
void BEMM4PL(double *data, int *CountNum, int *n_class, int *n_item, double *LH, 
			  ...
			  double *A, double *B, double *C, double *S, double *Tol, double *cr, int *E_exit,
			  double *SEA, double *SEB, double *SEC, double *SES, int *ParConstraint,   
			  ...
			  const double *PriorB, const double *PriorC, const double *PriorS, const int *n_Quadpts, 
	...
	for (i = 0; i<I; i++){
		...
		for (k = 0; k < nq; k++) {
			...
			for (j = 0; j < J; j++) { 
				...
				EZ = ((1 - S[j]) * Pstar[m]) / P[m] * data[n] + (S[j] * Pstar[m]) / ( 1 - P[m] )* ( 1 - data[n] );
	...
	while (E_exit[0] && (n_ECycle[0] < max_ECycle)) {
		for (j = 0; j < J; j++) {
			...
			if (PriorS[j]==-9 || PriorS[j + J]==-9) {				
				lss = - ( s_part1 / s_part6) - ( s_part2 / s_part5);	//lss=-(fz-rz)/s^2-rz/(1-s)^2
				st0 = s_part1 / s_part3;							    //s=(fz-rz)/fz
			} else {
				lss = - (( s_part1 + PriorS[j] -1) / s_part6) - (( s_part2+ PriorS[j + J] - 1.0) / s_part5);
				st0 = (PriorS[j] -1 + s_part1) / (((PriorS[j] + PriorS[j + J]) - 2.0) + s_part3);
			}
			...
			m = j + J * n_ECycle[0];
			if (isnormal(at0) && isnormal(bt0) && isnormal(ct0) && isnormal(st0)){
				if (ParConstraint[0]){
			...
			for (ParClass = 1; ParClass <= 4; ParClass++){
				...
				for (k = 0; k < nq; k++) {
					for (i = 0; i<I; i++){
						...
						for (j = 0; j < J; j++) { 
							...
							if (Pstar[v] >=1){Pstar[v] = 0.9999;}
							if (Pstar[v] <=0){Pstar[v] = 0.0001;}
							P[v] = deltahat_C[j] + (1-deltahat_S[j]-deltahat_C[j]) * Pstar[v];
							L *= P[v] * data[n] + (1 - P[v]) * (1 - data[n]);
						}
						v = i + I * k;
						LL[v] = L * weight_Quadpts[k];
				...
				for (i = 0; i<I; i++){
					for (k = 0; k < nq; k++) {
						...
						f[k] += Posterior_prob[v] * CountNum[i];
						r[m] += Posterior_prob[v] * data[n] * CountNum[i];
						EZ = ((1 - deltahat_S[jj]) * Pstar[m]) / P[m] * data[n] + (deltahat_S[jj] * Pstar[m]) / ( 1 - P[m] ) * ( 1 - data[n] );
						fz[m] += Posterior_prob[v] * EZ * CountNum[i];
						rz[m] += Posterior_prob[v] * EZ * data[n] * CountNum[i];
					}
				}
				...
				if (ParClass==4){
					...
					if (PriorS[jj]==-9 || PriorS[jj + J]==-9) {				
						st0 = s_part1 / s_part3;							    //s=(fz-rz)/fz
					} else {
						st0 = (PriorS[jj] -1 + s_part1) / (((PriorS[jj] + PriorS[jj + J]) - 2.0) + s_part3);
					}
					if (isnormal(st0)){
						if (ParConstraint[0]){
							if (st0>=0.0001 && st0<=0.5){
